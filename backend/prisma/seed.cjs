...
async function main() {
  ...
  async function createInvoice({ invoiceNo, buyer, productIdxs, status }) {
    ...
    return prisma.invoice.create({
      data: {
        invoiceNo,
        buyerId: buyer.id,
        supplierId: products[productIdxs[0]].supplierId,
        paymentMethod: 'Cash',
        total,
        status,
        items: { create: items },
  ...
  console.log('✅ Seed (CJS) completed successfully!')
}
