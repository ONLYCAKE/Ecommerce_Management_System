...
router.get('/', requirePermission(PERMISSIONS.PRODUCT_READ), async (req, res) => {
  try {
    ...
    const where = q ? { OR: [{ title: { contains: q as string, mode: 'insensitive' } }, { sku: { contains: q as string, mode: 'insensitive' } }] } : {};
    const items = await prisma.product.findMany({ where: where as any, include: { supplier: true }, orderBy: { createdAt: 'desc' as const } });
    res.json(items);
  } catch (err) {
...
router.post('/', requirePermission(PERMISSIONS.PRODUCT_CREATE), async (req, res) => {
  try {
    ...
    const data = { sku: dataIn.sku || null, title: dataIn.title?.trim(), description: dataIn.description || null, category: dataIn.category || null, price, stock, supplierId };
...
router.put('/:id', requirePermission(PERMISSIONS.PRODUCT_UPDATE), async (req, res) => {
  try {
    ...
    const data: any = { title: dataIn.title?.trim(), description: dataIn.description || null, category: dataIn.category || null, ...(price !== undefined && { price }), ...(stock !== undefined && { stock }), supplierId };
