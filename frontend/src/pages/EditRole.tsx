...
interface Role { id: number; name: string; permissions?: Record<string, boolean> }
...
export default function EditRole() {
  ...
  const toggle = async (key: string, current: boolean) => {
    try {
      ...
      setRole((r) => r ? { ...r, permissions: Object.fromEntries((data as string[]).map(k => [k, true])) } : r)
  ...
  if (!role) return <div className="p-8 text-gray-600">Loading role data...</div>
  ...
  return (
    <div className="p-8 min-h-screen bg-[var(--bg)] text-[var(--text)]">
            ...
            {modules.map((m) => (
              <tr key={m.base} className="border-t hover:bg-gray-50">
                ...
                {actions.map((action) => {
                  ...
                  return (
                    <td key={action}>
                      <input type="checkbox" className="h-4 w-4 accent-blue-600" checked={checked} disabled={disabled} onChange={() => toggle(key, checked)} />
                    </td>
                  )
                })}
              </tr>
      ...
      {toast && (
        <div className="fixed bottom-5 right-5 bg-green-600 text-white px-4 py-2 rounded-lg shadow-md">{toast}</div>
      )}
    </div>
  )
}
