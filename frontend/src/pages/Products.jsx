...
export default function Products() {
  ...
  useEffect(() => {
    (async () => {
      const [s, b] = await Promise.all([api.get('/suppliers'), api.get('/buyers')])
      setSuppliers(s.data)
      setBuyers(b.data)
    })()
  }, [])
  ...
  const open = (item = null) => {
    ...
    setForm(
      item
        ...
        : { sku: '', title: '', category: '', description: '', price: 0, stock: 0, supplierId: '', buyerId: '' }
    )
    setShowForm(true)
  }
        ...
        <table className="table w-full">
          <thead>
            <tr>
              ...
              <th className="cursor-pointer select-none" onClick={() => handleSort('stock')}>
          ...
          <tbody>
            {paged.map(p => (
              <tr key={p.id} className="border-t">
                ...
                <td className="text-sm text-gray-600">{p.buyer?.name || '-'}</td>
                <td className="text-sm text-gray-600">{p.supplier?.name || '-'}</td>
                <td>{formatINR(p.price)}</td>
                <td>{p.stock}</td>
                <td className="text-right space-x-2">
                  {canUpdate && <button className="btn-secondary" onClick={() => open(p)}>Edit</button>}
                  {canDelete && <button className="btn-danger" onClick={() => remove(p.id)}>Delete</button>}
                </td>
              </tr>
            ))}
            {filteredAndSorted.length === 0 && (
              <tr>
                <td colSpan="8" className="text-center text-gray-500 py-6">No products found.</td>
              </tr>
            )}
          </tbody>
        </table>
        {filteredAndSorted.length > 0 && (
          <div className="mt-4 flex justify-end">
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-600">Page {page} of {Math.max(1, Math.ceil(filteredAndSorted.length / pageSize))}</span>
              ...
              <button className="btn-secondary" disabled={page>=Math.ceil(filteredAndSorted.length/pageSize)} onClick={()=>setPage(p=>Math.min(Math.ceil(filteredAndSorted.length/pageSize)||1,p+1))}>{'>'}</button>
              <button className="btn-secondary" disabled={page>=Math.ceil(filteredAndSorted.length/pageSize)} onClick={()=>setPage(Math.max(1,Math.ceil(filteredAndSorted.length/pageSize)))}>{'>>'}</button>
            </div>
          </div>
        )}
      </div>

      {/* Product Modal (unchanged layout) */}
      {showForm && (
            ...
            <form onSubmit={save} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input className={`input mt-1 ${errors.sku ? 'border-red-500' : ''}`} value={form.sku} onChange={e => setForm(f => ({ ...f, sku: e.target.value }))} />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select className={`input mt-1 ${errors.buyerId ? 'border-red-500' : ''}`} value={form.buyerId} onChange={e => setForm(f => ({ ...f, buyerId: e.target.value }))}>
                    <option value="">Select Buyer</option>
                    {buyers.map(b => (<option key={b.id} value={b.id}>{b.name}</option>))}
                  </select>
                  {errors.buyerId && <div className="text-red-500 text-xs mt-1">{errors.buyerId}</div>}
                </div>
                <div>
