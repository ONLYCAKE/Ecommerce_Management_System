...
export default function Orders() {
  ...
  // ✅ Updated version — opens invoice in new tab for preview (no auto download)
  const previewPdf = async (invNo) => {
    try {
      ...
      window.open(fileURL, '_blank') // ✅ Opens new tab for preview
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="btn-primary" onClick={() => navigate('/invoices?new=1')}>
            + Add Invoice
          </button>
        </div>
      </div>
                ...
                <tr key={inv.id} className="border-t hover:bg-[rgba(255,255,255,0.05)] transition-all">
                  ...
                  <td className="text-center">
                    <button
                      ...
                      onClick={() => previewPdf(inv.invoiceNo)}
                    >
                      View / Download
                    </button>
                  </td>
