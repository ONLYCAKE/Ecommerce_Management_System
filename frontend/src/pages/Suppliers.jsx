...
export default function Suppliers() {
  ...
  const fetchAll = async () => {
    const { data } = await api.get('/suppliers', { params: { q } })
    setItems(data)
  }
  ...
  const validate = () => {
    ...
    // ✅ Email validation: lowercase + @gmail.com
    if (!form.email) {
      ...
      <div className="card p-4">
        ...
        {items.length > 0 && (
          <div className="flex items-center justify-between mt-4">
            <div className="text-sm text-gray-600">Page {page} of {Math.max(1, Math.ceil(items.length / pageSize))}</div>
